...
{
public:
    CWLookAndFeel (const juce::Drawable& knobDrawable, const juce::Drawable& knobShadowDrawable) : knob (knobDrawable.createCopy()), knobShadow(knobShadowDrawable.createCopy())
    {
                           ...
                           float, float, juce::Slider&) override
    {
        ...
        auto shadowTransform = juce::AffineTransform::scale (bounds.getWidth() / knobShadow->getWidth())
                                        .translated (shiftX, shiftY);
